...
def sshDeploy(retry,hostname):
    ...
    try:
        ...
        session.get_pty()
        command="cd InsightAgent-testing && sudo ./install.sh -u "+userInsightfinder+" -k "+licenseKey+" -s "+samplingInterval+" -r "+reportingInterval+" -t "+agentType
...
def get_args():
    ...
    parser.add_argument(
        '-t', '--AGENT_TYPE', type=str, help='Agent type: proc or cadvisor or docker_remote_api or cgroup or daemonset or hypervisor', choices=['proc', 'cadvisor', 'docker_remote_api', 'cgroup', 'daemonset', 'hypervisor'], required=True)
    parser.add_argument(
